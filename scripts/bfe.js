...
const extractProblemInfo = async () => {
    ...
    return {
        ...
        problemName: JSON.parse(QUESTION_INFO)['props']['pageProps']['item']['title'],
        problemLevel: JSON.parse(QUESTION_INFO)['props']['pageProps']['item']['difficulty'],
        problemUrl: `https://bigfrontend.dev/problem/${JSON.parse(QUESTION_INFO)['query']['permalink']}`
    };
}
...
const monitorSubmissionResult = () => {
    ...
    const functionId = setInterval(async () => {
        ...
        submissionResult = document.querySelector("#__next > div.Page__Outer-sc-90ozb8-0.cCuUVy > main > div.ProblemDetailJS__Judge-ffiw7z-3.dxhNjB > div.ProblemDetailJS__JudgeFooter-ffiw7z-5.enghjr > div.ProblemDetailJS__Logs-ffiw7z-6.ezuvBO > p.TestResultView__Success-sc-20rtgt-0.fzUVDA");
        ...
        clearInterval(functionId);
        let isSuccess = submissionResult.innerText.includes('👍 well done! all cases passed');
