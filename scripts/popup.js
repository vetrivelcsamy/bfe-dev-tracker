...
const getNextReviewTime = (problem) => {
  return new Date(
    problem.submissionTime + forggetingCurve[problem.proficiency] * 60 * 1000
  );
};
...
const getProblemUrlCell = (problem, width) =>
  `<td style="width: ${width | 30}%;"><a target="_blank" href=${
    problem.url
  }><small>${problem.name}</small><a/></td>`;
const getProblemProgressBarCell = (problem, width) => {
  return `\
    <td style="width: ${width | 10};">\
        <div class="progress" role="progressbar" aria-label="Success example" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">\
            <div class="progress-bar progress-bar-striped bg-success" style="width: ${
              (Math.max(problem.proficiency, 0) / 5) * 100
            }%; font-size: smaller; color: black"><small><small><small>${
    (problem.proficiency / 5) * 100
  }%</small></small></small></div>\
        </div>\
    </td>\
    `;
};
const getProblemLevelCell = (problem, width) =>
  `<td style="width: ${width | 12}%;"><small>${decorateProblemLevel(
    problem.level
  )}</small></td>`;

const getCheckButtonTag = (
  problem
) => `<small class="fa-regular fa-square-check fa-2xs mt-2 mb-0 check-btn-mark"\ 
...
const getDeleteButtonTag = (
  problem
) => `<small class="fa-regular fa-square-minus fa-2xs mt-2 mb-0 delete-btn-mark"\ 
...
const getResetButtonTag = (
  problem
) => `<small class="fa-solid fa-arrows-rotate fa-2xs mt-2 mb-0 reset-btn-mark" \
...
const create_review_problem_record = (problem) => {
  ...
  const htmlTag = `\
    <tr>\
        ...
        <td><small>${Math.round(
          (Date.now() - nextReviewDate) / (60 * 60 * 1000)
        )} hour(s)</small></td>\
...
const create_schedule_problem_record = (problem) => {
  ...
  const htmlTag = `\
    <tr style="vertical-align:middle">\
        ...
        ${getProblemLevelCell(problem)}\
        <td><small>${
          months[nextReviewDate.getMonth()]
        } ${nextReviewDate.getDate()} ${nextReviewDate.getHours()}:${
    nextReviewDate.getMinutes() < 10
      ? `0${nextReviewDate.getMinutes()}`
...
const update_review_table_content = (problems, page) => {
  ...
  if (page === toReviewMaxPage)
    nextButton0DOM.setAttribute("disabled", "disabled");
...
const update_completed_table_content = (problems, page) => {
  ...
  if (page === completedMaxPage)
    nextButton2DOM.setAttribute("disabled", "disabled");
...
const mark_problem_as_mastered = async (event) => {
  ...
  const queryKey = false ? "cn_records" : "bfe_records";
...
const update_record_operation_event_listener = () => {
  ...
  if (checkButtons !== undefined) {
    Array.prototype.forEach.call(
      checkButtons,
      (btn) => (btn.onclick = mark_problem_as_mastered)
    );
  }
  ...
  if (deleteButtons !== undefined) {
    Array.prototype.forEach.call(
      deleteButtons,
      (btn) => (btn.onclick = delete_problem)
    );
  }
...
const display_table = () => {
  chrome.storage.local.get("cn_mode", (result) => {
    ...
    if (false) {
      labelDom.innerHTML = "Bfe.dev - China ";
    } else {
      labelDom.innerHTML = "Bfe.dev Tracker";
    }
    ...
    chrome.storage.local.get(queryKey, (result) => {
      ...
      reviewScheduledProblems = problems.filter(
        (p) => !needReview(p) && p.proficiency < 5
      );
      completedProblems = problems.filter((p) => p.proficiency === 5);

      toReviewMaxPage = Math.max(
        Math.ceil(needReviewProblems.length / PAGE_SIZE),
        1
      );
      scheduledMaxPage = Math.max(
        Math.ceil(reviewScheduledProblems.length / PAGE_SIZE),
        1
      );
      completedMaxPage = Math.max(
        Math.ceil(completedProblems.length / PAGE_SIZE),
        1
      );

      needReviewProblems.sort((p1, p2) => {
        return (
          getNextReviewTime(p1).valueOf() - getNextReviewTime(p2).valueOf()
        );
      });
